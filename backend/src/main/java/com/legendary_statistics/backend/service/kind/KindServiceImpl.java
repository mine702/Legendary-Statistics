    ...
    public List<GetKindRes> getKindList() {
        return kindRepository.findAll().stream().map(
                kindEntity -> GetKindRes.builder()
                        .id(kindEntity.getId())
                        .actualFileName(Optional.ofNullable(kindEntity.getFileLegendEntity()).map(FileLegendEntity::getActualFileName).orElse("파일 없음"))
                        .path(Optional.ofNullable(kindEntity.getFileLegendEntity()).map(FileLegendEntity::getPath).orElse("경로 없음"))
