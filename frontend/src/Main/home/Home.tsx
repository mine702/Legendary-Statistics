        ...
        <main className={style.mid}>
          <div className={style.container}>
            ...
            <div className={style.announcement}>
              <p>TFT META 는 라이엇 게임즈의 지식재산 이용 정책에 따라</p>
              <p>라이엇 게임즈 소유의 자산을 이용하여 제작되었습니다.</p>
              <p>라이엇 게임즈는 이 프로젝트를 지지하거나 후원하지 않습니다.</p>
            </div>
            ...
            <div className={style.rankingContainer} onClick={() => navigate("/ranking")}>
              <div className={style.description}>좋아하는 꼬마 전설이의 랭킹을 확인해보세요!</div>
              <div className={style.rankingContent}>
                {ranking?.items ? (
                  <>
                    <RankingCard rank={2} image={ranking.items[1].path} name={ranking.items[1].name}/>
                    <RankingCard rank={1} image={ranking.items[0].path} name={ranking.items[0].name}/>
                    <RankingCard rank={3} image={ranking.items[2].path} name={ranking.items[2].name}/>
                  </>
                ) : (
            ...
            <div className={style.legendContainer}>
              <div className={style.description}>새롭게 추가된 꼬마 전설이 리스트 입니다!</div>
              <div className={style.legendContent}>
                {!lastLegend ? (
                  <div>꼬마 전설이 정보를 불러오는 중입니다...</div>
                ) : (
                  lastLegend.map((item) => <LegendCard key={item.name} legend={item}/>)
                )}
              </div>
            </div>
