...
import {GetChampionRes, GetRerollProbabilityRes, GetSeasonRes, GetSynergyRes} from "./dto/reroll.ts";
...
export const useSWRGetChampionList = (seasonId?: number) => {
    const key = seasonId ? `/reroll/champion-list/${seasonId}` : null;
    return useSWR<GetChampionRes[]>(key, defaultFetchAxios);
}

export const useSWRGetSynergyList = (seasonId?: number) => {
    const key = seasonId ? `/reroll/synergy-list/${seasonId}` : null;
    return useSWR<GetSynergyRes[]>(key, defaultFetchAxios);
}
